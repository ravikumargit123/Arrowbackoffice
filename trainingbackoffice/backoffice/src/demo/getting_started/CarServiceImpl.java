...
public class CarServiceImpl implements CarService
{
	...
	private final List<Car> cars = new ArrayList<Car>(20);
	private final Map<String, List<Car>> queryResults;
	//private Category rootCategory;

	public CarServiceImpl()
	{

		final Random r = new Random(0L); // random but same data
		final String[][] raw =
		{
				{ "China", "Geely", "Panda", "Sedan" },
				{ "France", "Peugeot", "RCZ", "Sport" },
				{ "France", "Renault", "Megane", "Sedan" },
				{ "Germany", "Audi", "TT", "Sport" },
				{ "Germany", "BMW", "X3", "SUV" },
				{ "Germany", "Mercedes-Benz", "Sprinter", "Van" },
				{ "Germany", "Volkswagen", "Transporter", "Van" },
				{ "Germany", "Porsche", "Cayenne", "SUV" },
				{ "Italy", "Ferrari", "F430", "Supercar" },
				{ "Italy", "Lamborghini", "Gallardo", "Supercar" },
				{ "Japan", "Toyota", "Camry", "Sedan" },
				{ "Japan", "Mitsubishi", "Pajero", "SUV" },
				{ "Japan", "Nissan", "GT-R", "Sport" },
				{ "Japan", "Subaru", "Legacy", "MPV" },
				{ "Korea", "Hyundai", "Tucson", "SUV" },
				{ "Korea", "Kia", "Rio", "Sedan" },
				{ "Sweden", "Volvo", "S40", "Sedan" },
				{ "Taiwan", "LUXGEN", "Luxgen7", "MPV" },
				{ "United States", "Ford", "Focus", "Sedan" },
				{ "United Kingdom", "Land Rover", "Discovery", "SUV" } };
		final int[] disp = new int[]
		{ 1600, 1800, 2000, 2400, 3000, 3200, 3500 }; // engine displacement
		final double[] cost = new double[]
		{ 19999.9, 24999.9, 29999.9, 34999.9, 39999.9 }; // cost
		...
		//		}
		for (final Car car : findAll())
		{
			final String typeFilter = "type = " + car.getType();
			queryResults.get(typeFilter).add(car);
			final String typeDispFilter = car.getEngineDisplacement() > 2000 ? " & displacement > 2000" : " & displacement <= 2000";
	...
	public int countByFilter(final String filter)
	{
		// fake count
		return queryByFilter(filter).size();
	}

	public List<Car> queryByFilter(final String filter)
	{
		// fake query
